...
class SmoothFont : public Font {
 public:
  ...
  SmoothFont(const U *font_data PROGMEM)
      : SmoothFont((const roo_io::byte *PROGMEM)font_data) {}

  SmoothFont(const roo_io::byte *font_data PROGMEM);
 ...
 private:
  ...
  const roo_io::byte *PROGMEM findGlyph(char32_t code) const;
  const roo_io::byte *PROGMEM findKernPair(char32_t left, char32_t right) const;
  ...
  void drawGlyphModeFill(DisplayOutput &output, int16_t x, int16_t y,
                         ...
                         const roo_io::byte *PROGMEM data, int16_t offset,
                         const Box &clip_box, Color color, Color bgColor,
                         BlendingMode blending_mode) const;
  ...
  const roo_io::byte *glyph_metadata_begin_ PROGMEM;
  const roo_io::byte *glyph_kerning_begin_ PROGMEM;
  const roo_io::byte *glyph_data_begin_ PROGMEM;
};
