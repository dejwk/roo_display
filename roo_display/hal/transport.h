...
class GenericSpi {
 public:
  // Creates the SPI transport, with specified transaction settings, using the provided
  // SPI bus.
  GenericSpi(decltype(SPI)& spi, SPISettings settings)
      : spi_(spi), settings_(std::move(settings)) {}

  // Creates the SPI transport, with specified clock, and using MSBFIRST and SPI_MODE0,
  // using the provided SPI bus.
  GenericSpi(decltype(SPI)& spi, uint32_t clock)
      : spi_(spi), settings_(clock, MSBFIRST, SPI_MODE0) {}

  // Creates the SPI transport, with specified transaction settings, using the default
  // SPI bus.
  ...
  void writeBytes(uint8_t* data, uint32_t size) { spi_.writeBytes(data, size); }
...
// Convenience template wrapper that allows to capture all SPI configuration settings
// at compile time, inside a class specialization.
