...
Display display(hardware.display(), hardware.touch(), hardware.touchCalibration());
...
void setup() {
  ...
  // Initialize hardware (display and touch controller)
  if (!hardware.init()) {
  ...
  dc.draw(TextLabel("***    Just a very basic test    ***", font_NotoSans_Regular_27(), color::Cyan),
          50, 50);

  // Draw six different shapes to demonstrate display capabilities
  dc.draw(FilledCircle::ByExtents(startX, centerY - 50, 50, color::Red));
  dc.draw(FilledRect(startX + spacing - 50, centerY - 50,
                     startX + spacing + 50, centerY + 50, color::Green));
  dc.draw(Rect(startX + 2 * spacing - 50, centerY - 50,
               startX + 2 * spacing + 50, centerY + 50, color::Blue));
  dc.draw(Circle::ByExtents(startX + 3 * spacing, centerY - 50, 50, color::Cyan));
  dc.draw(FilledRect(startX + 4 * spacing - 50, centerY - 50,
                     startX + 4 * spacing + 50, centerY + 50, color::Magenta));
  dc.draw(Line(startX + 5 * spacing - 50, centerY - 50,
               startX + 5 * spacing + 50, centerY + 50, color::Yellow));
  dc.draw(Line(startX + 5 * spacing + 50, centerY - 50,
               startX + 5 * spacing - 50, centerY + 50, color::Yellow));

  // Draw reset button in top-right corner
  dc.draw(FilledRect(RESET_BTN_X, RESET_BTN_Y,
                     RESET_BTN_X + RESET_BTN_W, RESET_BTN_Y + RESET_BTN_H,
                     color::DarkRed));
  dc.draw(Rect(RESET_BTN_X, RESET_BTN_Y,
               RESET_BTN_X + RESET_BTN_W, RESET_BTN_Y + RESET_BTN_H,
               color::White));
  dc.draw(TextLabel("Reset", font_NotoSans_Regular_27(), color::White),
          RESET_BTN_X + 25, RESET_BTN_Y + 38);

  dc.draw(TextLabel("Open serial console and touch screen ...", font_NotoSans_Regular_27(), color::White),
          50, 420);

  Serial.println("Ready! Touch test active.\n");
  Serial.println("Touch coordinates format: [count] (x, y) @ time_ms [region]");
  Serial.println("------------------------------------------------");
}
...
void loop() {
  ...
  if (display.getTouch(x, y)) {
    ...
    if (x >= RESET_BTN_X && x <= (RESET_BTN_X + RESET_BTN_W) && y >= RESET_BTN_Y && y <= (RESET_BTN_Y + RESET_BTN_H)) {
    ...
    if (y < 100) region = "top";
    else if (y > 380) region = "bottom";
    else if (y > 140 && y < 340) region = "shapes-row";
    ...
    DrawingContext dc(display);
    dc.draw(FilledCircle::ByExtents(x - 10, y - 10, 20, color::Yellow));
