...
// Reads raw SPI register state useful for diagnostics.
inline uint32_t SpiGetMosiDlenRaw(uint8_t spi_port)
    __attribute__((always_inline));
inline uint32_t SpiGetUserRaw(uint8_t spi_port) __attribute__((always_inline));
inline uint32_t SpiGetCmdRaw(uint8_t spi_port) __attribute__((always_inline));
inline uint32_t SpiGetDmaConfRaw(uint8_t spi_port)
    __attribute__((always_inline));
...
// inline bool SpiDmaTxActive(uint8_t spi_port) {
//   return (READ_PERI_REG(SPI_DMA_STATUS_REG(spi_port)) & SPI_DMA_TX_EN) != 0;
// }

// inline void SpiDmaFifoReset(uint8_t spi_port) {
//   constexpr uint32_t kDmaFifoResetMask = SPI_AHBM_RST | SPI_AHBM_FIFO_RST;
//   SET_PERI_REG_MASK(SPI_DMA_CONF_REG(spi_port), kDmaFifoResetMask);
//   CLEAR_PERI_REG_MASK(SPI_DMA_CONF_REG(spi_port), kDmaFifoResetMask);
// }
...
inline uint32_t SpiGetMosiDlenRaw(uint8_t spi_port) {
  return READ_PERI_REG(SPI_MOSI_DLEN_REG(spi_port));
}

inline uint32_t SpiGetUserRaw(uint8_t spi_port) {
  return READ_PERI_REG(SPI_USER_REG(spi_port));
}

inline uint32_t SpiGetCmdRaw(uint8_t spi_port) {
  return READ_PERI_REG(SPI_CMD_REG(spi_port));
}

inline uint32_t SpiGetDmaConfRaw(uint8_t spi_port) {
  return READ_PERI_REG(SPI_DMA_CONF_REG(spi_port));
}
